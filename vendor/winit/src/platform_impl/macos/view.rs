...
declare_class!(
    ...
    unsafe impl WinitView {
        ...
        // macOS 26 crash fix: _NSTouchBarFinder walks the NSResponder chain
        // looking for a touch bar provider.  On each view it visits it calls
        // -addObserver:forKeyPath:options:context: for the "nextResponder"
        // key so it can be notified when the chain changes.  When the chain
        // does change, multiple "invalidate" blocks get scheduled on the main
        // thread by ___NSTouchBarFinderSetNeedsUpdateOnMain_block_invoke_2.
        // The second block tries to call -removeObserver:forKeyPath:context:
        // for an observer that the first block already removed, producing:
        //
        //   NSRangeException: Cannot remove an observer
        //   <_NSTouchBarFinderObservation 0x…> for the key path
        //   "nextResponder" from <WinitView 0x…> because it is not
        //   registered as an observer.
        //
        // Fix: make all three KVO mutation methods no-ops on WinitView.
        // WinitView is winit's private internal view class; no user code,
        // bindings, or accessibility framework legitimately KVO-observes it.
        // The only real caller is _NSTouchBarFinder.  With no observers ever
        // registered, the "remove" calls always succeed silently and the
        // crash cannot occur.
        //
        // Note: we do NOT call super here by design.  Using
        // `msg_send![super(self), …]` without an explicit class argument
        // in objc2 0.5.x re-dispatches to WinitView itself (the same class),
        // causing infinite recursion and a stack-overflow panic inside an
        // extern-C function — which aborts rather than unwinding.
