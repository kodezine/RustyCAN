...
| CN3 | CAN DB9 → TJA1044 transceiver | PA12 (TX) / PA11 (RX) — confirmed from schematic |
| CN14 | USB OTG HS (ULPI) | USB3320C-EZK PHY; connected to host Mac |
| CN18 | USB OTG FS Micro-AB | not used (USB-HS via CN14) |
| CN16 | OTG1 FS | not used |

> **Schematic confirmed (2026-05-02):** FDCAN1_RX = PA11, FDCAN1_TX = PA12. Fixed in `main.rs`.
...
- [x] Flash default build: `cargo run --release` (from `firmware/dongle-h743/`)
- [x] Connect USB-HS cable: host Mac → **CN14** (OTG HS ULPI)
- [x] RTT: `KCAN Dongle v1 (H743I) — booting`
- [x] macOS ioreg shows device: VID=0x1209, PID=0xBEEF, serial=00390037
- [x] USBSpeed=3 (480 Mbps High-Speed via USB3320C-EZK ULPI PHY)

**Result:** Enumerates as "KCAN Dongle v1 (H743I)", Kodezine, serial=00390037, 480 Mbps (USB HS via ULPI — full hardware speed). Note: `vbus_detection = false`; barrel-jack power recommended.
...
- [x] `KCanEp0Handler` implemented in `firmware/dongle-h743/src/ep0_handler.rs`
- [x] GET_INFO, GET_BT_CONST, SET_BITTIMING, SET_MODE all ACK'd
- [x] BULK_RESTART + EP1 FIFO flush (24w, DATA0) fires on every SET_MODE
- [x] CLEAR_FEATURE on EP1 OUT and EP81 IN both accepted
- [x] `KCanAdapter::open()` at 250 kbps succeeds without error
...
- [x] Bulk IN (device→host): live CANopen frames from real slave node decoded correctly
  — NMT heartbeats @ 1 Hz (node 32), EMCY frame, sustained session > 2 minutes
- [x] No session hangs or USB timeouts over soak duration
- [x] CANopen NMT / EMCY decoded correctly by rustycan host layer
...
- [x] Host-side disconnect detection: `TransferError::Disconnected` → `AdapterError::Disconnected`
...
6. **CLEAR_FEATURE race (h743):** After `BULK_RESTART`, firmware immediately wrote the first
   queued frame (EP1 `EPENA=1`). macOS `clear_halt()` arrived while `EPENA=1`; Embassy's
   `endpoint_set_stalled(false)` wrote `SD0PID` while `EPENA=1` — violates STM32H7 RM §
   56.8.6 — corrupting the endpoint permanently → `kIOReturnBadArgument` on every subsequent
   bulk-IN call. Fix: 150 ms `Timer::after()` in the `BULK_RESTART` handler keeps EP1 idle
   until after `CLEAR_FEATURE` completes (commit `cac92d9`).
