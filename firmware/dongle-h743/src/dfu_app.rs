...
pub async fn dfu_app_task(flash_peri: Peri<'static, FLASH>) {
    ...
    // Always confirm the running image before accepting a DFU request.
    // mark_booted() involves a flash erase+write (~2 s); if DFU_DETACH
    // arrives during that window the old select() would race: mark_dfu()
    // would call verify_booted() while STATE is still SWAP_MAGIC, fail,
    // and reset without writing DFU_DETACH_MAGIC — causing the bootloader
    // to see SWAP_MAGIC+already-swapped and revert instead of DFU.
    // Waiting for MARK_BOOTED first eliminates this race entirely.
    MARK_BOOTED.wait().await;
    {
    ...
    ENTER_DFU.wait().await;
    {
        info!("DFU app: writing DFU magic — rebooting");
        // Notify the LCD before the blocking flash write so the display
        // task can update during the subsequent Timer yield.
        lcd_terminal::boot_log!(
            crate::display_task::LOG_CHANNEL,
            "Entering DFU mode — USB bootloader active",
            lcd_terminal::BootStatus::Warn
        );
        crate::display_task::USB_STATUS.signal(crate::display_task::UsbDisplayStatus::EnteringDfu);
        let state_part = BlockingPartition::new(&flash_mutex, STATE_OFFSET, STATE_SIZE);
        let mut aligned = AlignedBuffer([0u8; FLASH_WRITE_SIZE]);
        let mut state = BlockingFirmwareState::new(state_part, aligned.as_mut());
