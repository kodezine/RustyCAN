...
//! | Signal        | Pin  | Notes                                           |
//! |---------------|------|-------------------------------------------------|
//! | FDCAN1_RX     | PH14 | On-board TJA1044 transceiver → CN3 DB9 (ch 0)  |
//! | FDCAN1_TX     | PH13 | On-board TJA1044 transceiver → CN3 DB9 (ch 0)  |
//! | USB_OTG_FS_DM | PA11 | CN18 Micro-AB connector                         |
//! | USB_OTG_FS_DP | PA12 | CN18 Micro-AB connector                         |
//! | LD1 (green)   | PF10 | Heartbeat — 1 Hz blink = firmware alive         |
//! | LD3 (orange)  | PA4  | USB host connected (solid on = enumerated)      |
//!
//! # Clocks
//!
//! System clock: 480 MHz (HSE 25 MHz + PLL1).
...
async fn main(spawner: Spawner) {
    ...
    // ── FDCAN1: channel 0 — on-board TJA1044 → CN3 DB9 (PH13/PH14) ──────────
    let mut can1_cfg = can::CanConfigurator::new(p.FDCAN1, p.PH14, p.PH13, Irqs);
