/* STM32H743XI bootloader linker script.
 *
 * The bootloader lives in Bank1 sector 0 (128 KB, 0x08000000–0x0801FFFF).
 *
 * Partition layout (full 2 MB flash):
 *   BOOTLOADER  : 0x08000000  128 KB  Bank1 sector 0   — this binary
 *   ACTIVE      : 0x08020000  768 KB  Bank1 sectors 1–6 — KCAN app
 ...
 *
 * embassy-boot requires DFU >= ACTIVE + page_size: 896K - 768K = 128K = page_size ✓
 *
 * The __bootloader_* symbols are required by embassy-boot's
 * BootLoaderConfig::from_linkerfile_blocking and
 * FirmwareUpdaterConfig::from_linkerfile_blocking.
 * They are offsets from the start of FLASH (0x08000000).
 */

MEMORY
{
    FLASH  (rx)  : ORIGIN = 0x08000000, LENGTH = 128K  /* bootloader code */
